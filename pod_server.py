...
DTYPE = torch.float16 if IS_ROCM else torch.float16
...
def load_qwen_image():
    ...
    if "qwen_image" not in _models:
        ...
        try:
            ...
            pipe.to(DEVICE)  # Keep on GPU - no CPU offload on high-VRAM GPUs
...
def load_qwen_image_edit():
    ...
    if "qwen_image_edit" not in _models:
        ...
        try:
            clear_vram()
            print(f"Loading Qwen-Image-Edit-2511 on {DEVICE} ({'ROCm' if IS_ROCM else 'CUDA'})...")
...
def load_qwen_image_edit_plus():
    ...
    if "qwen_image_edit_plus" not in _models:
        ...
        try:
            clear_vram()
            print(f"Loading Qwen-Image-Edit-2511 Plus on {DEVICE} ({'ROCm' if IS_ROCM else 'CUDA'})...")
...
def load_qwen_image_edit_with_angles_lora(lora_strength: float = 0.9):
    ...
    if "qwen_image_edit_angles" not in _models:
        ...
        try:
            clear_vram()
            print(f"Loading Qwen-Image-Edit-2511 with Multiple Angles LoRA on {DEVICE}...")
            from diffusers import QwenImageEditPipeline
...
if __name__ == "__main__":
    print("Starting StoryGen GPU Server (ROCm/AMD) with job queue...")
